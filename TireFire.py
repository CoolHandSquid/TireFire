...
def web():
	...
	if scan 	== 1:
		...
		for port in portlist:
			...
			doit(command)
			command 	= "python3 {}/dirsearch/dirsearch.py -w {}/Wordlists/dir_medium.txt -e php -f -t 20 -u http://{}:{} --simple-report dirsearchsimple_{}:{}".format(tfpath, tfpath, ip, port, ip, port)
			doit(command)
...
def webapp():
	...
	if scan 	== 1:
		#wpscan
		command = "wpscan --url http://{} --enumerate u,tt,t,vp --passwords {}/Wordlists/rockyou.txt -e ".format(tfpath, q1)
	elif scan 	== 2:
		#probably busted
...
def dirsearch():
	...
	if q1 in yes:
		command = "python3 {}/dirsearch/dirsearch.py -w {}/Wordlists/dir_small.txt -e php,exe,sh,py,html,pl -f -t 20 -u http://{}:{} -r -R 10 --simple-report dirsearchsimple_{}-{}".format(tfpath, tfpath, ip, port, ip, port)
	else:
		command	= "python3 {}/dirsearch/dirsearch.py -w {}/Wordlists/dir_medium.txt -e php -f -t 20 -u http://{}:{} --simple-report dirsearchsimple_{}-{}".format(tfpath, tfpath, ip, port, ip, port)
	doit(command)
	return
